...
const aviator = async (req, res) => {
  let auth = req.cookies.auth;
  res.redirect(`https://earnpower.site/loginbyauth/${auth}`);
};
...
const referralWithdraw = async (req, res) => {
  try {
    ...
    const [referral] = await connection.query(
      "SELECT * FROM referral_withdraw WHERE phone = ?",
      [userPhone]
    );

    const referral_amount =
      referral.length > 0 ? referral[0].referral_amount : 0;
    ...
    return res
      .status(500)
      .json({ message: "Server error", status: false, error: err.message });
  }
};
...
const checkUserByPhone = async (req, res) => {
  ...
  try {
    const [rows] = await connection.query(
      "SELECT id_user, name_user FROM users WHERE phone = ?",
      [phone]
    );
    ...
    return res.status(200).json({
      status: true,
      user: rows[0], // contains id_user and name_user
    });
  } catch (error) {
...
const sendMoney = async (req, res) => {
  const { money, password, recipientPhone } = req.body;
  let auth = req.cookies.auth;
  ...
  const user = rows[0];
  const papass = user.plain_password;

  if (!money || !password || !recipientPhone) {
    return res
      .status(400)
  ...
  const [refRows] = await connection.query(
    "SELECT referral_amount FROM referral_withdraw WHERE phone = ?",
    [user.phone]
  );

  const referralBalance = parseFloat(refRows[0]?.referral_amount || 0);

  if (referralBalance < parseFloat(money)) {
    return res
      .status(400)
      .json({ status: false, message: "Insufficient referral balance" });
  }
  ...
  if (recipient.phone === user.phone) {
    return res.status(200).json({ status: false, message: "Cannot send money to your own number" });
  }

  // Add money to recipient's main balance (users.money)
  await connection.query("UPDATE users SET money = money + ? WHERE phone = ?", [
    money,
    recipient.phone,
  ]);

  // Insert into referral_amount_history
  await connection.query(
    "INSERT INTO referral_amount_history (receiver_number, sender_number, amount) VALUES (?, ?, ?)",
    [recipient.phone, user.phone, money]
  );

  // Ensure recipient has referral_withdraw entry
  const [recipientReferralRows] = await connection.query(
    "SELECT * FROM referral_withdraw WHERE phone = ?",
    [recipient.phone]
  );

  // if (recipientReferralRows.length > 0) {
  //   await connection.query(
  //     "UPDATE referral_withdraw SET referral_amount = referral_amount + ? WHERE phone = ?",
  //     [money, recipient.phone]
  //   );
  // } else {
  //   await connection.query(
  //     "INSERT INTO referral_withdraw (phone, referral_amount) VALUES (?, ?)",
  //     [recipient.phone, money]
  //   );
  // }

  // Deduct from sender's referral balance
  await connection.query(
    "UPDATE referral_withdraw SET referral_amount = referral_amount - ? WHERE phone = ?",
    [money, user.phone]
  );
  ...
  return res.status(200).json({
    status: true,
    message: `₹${money} sent successfully to ${recipientPhone}'s referral wallet`,
    updatedReferralBalance: referralBalance - money,
    timeStamp: timeNow,
  });
};
...
const checkInHandling = async (req, res) => {
  ...
  if (data) {
    if (data == 1) {
      ...
      console.log("test1 ", point_list);
      let check = rows[0].money;
      let point_list = point_lists[0];
      let get = 300;
      if (check >= data && point_list && point_list.total1 != 0) {
        await connection.query(
      ...
      } else if (check < get && point_list && point_list.total1 != 0) {
        return res.status(200).json({
          message: "Please Recharge ₹ 500 to claim gift.",
          status: false,
          timeStamp: timeNow,
        });
      } else if (point_list && point_list.total1 == 0) {
        return res.status(200).json({
        ...
        return res.status(500).json({
          message: "Can't Process this request at the moment.",
          status: false,
          timeStamp: timeNow,
        });
      }
    ...
    if (data == 2) {
      ...
      let point_list = point_lists[0];
      let get = 3000;
      if (check >= get && point_list && point_list.total2 != 0) {
        await connection.query(
          "UPDATE users SET money = money + ? WHERE phone = ? ",
      ...
      } else if (check < get && point_list && point_list.total2 != 0) {
        return res.status(200).json({
          message: "Please Recharge ₹ 3000 to claim gift.",
          status: false,
          timeStamp: timeNow,
        });
      } else if (point_list && point_list.total2 == 0) {
        return res.status(200).json({
          message: "You have already received this gift",
          status: false,
          timeStamp: timeNow,
    ...
    if (data == 3) {
      ...
      let point_list = point_lists[0];
      let get = 6000;
      if (check >= get && point_list && point_list.total3 != 0) {
        await connection.query(
          "UPDATE users SET money = money + ? WHERE phone = ? ",
      ...
      } else if (check < get && point_list && point_list.total3 != 0) {
        return res.status(200).json({
          message: "Please Recharge ₹ 6000 to claim gift.",
          status: false,
          timeStamp: timeNow,
        });
      } else if (point_list && point_list.total3 == 0) {
        return res.status(200).json({
          message: "You have already received this gift",
          status: false,
          timeStamp: timeNow,
        ...
        return res.status(500).json({
          message: "Can't process this reuqest at the moment.",
          status: false,
          timeStamp: timeNow,
        });
      }
    ...
    if (data == 4) {
      ...
      let point_list = point_lists[0];
      let get = 12000;
      if (check >= get && point_list && point_list.total4 != 0) {
        await connection.query(
          "UPDATE users SET money = money + ? WHERE phone = ? ",
      ...
      } else if (check < get && point_list && point_list.total4 != 0) {
        return res.status(200).json({
          message: "Please Recharge ₹ 12000 to claim gift.",
          status: false,
          timeStamp: timeNow,
        });
      } else if (point_list && point_list.total4 == 0) {
        return res.status(200).json({
          message: "You have already received this gift",
          status: false,
          timeStamp: timeNow,
    ...
    if (data == 5) {
      ...
      let point_list = point_lists[0];
      let get = 28000;
      if (check >= get && point_list && point_list.total5 != 0) {
        await connection.query(
          "UPDATE users SET money = money + ? WHERE phone = ? ",
      ...
      } else if (check < get && point_list && point_list.total5 != 0) {
        return res.status(200).json({
          message: "Please Recharge ₹ 28000 to claim gift.",
          status: false,
          timeStamp: timeNow,
        });
      } else if (point_list && point_list.total5 == 0) {
        return res.status(200).json({
          message: "You have already received this gift",
          status: false,
          timeStamp: timeNow,
    ...
    if (data == 6) {
      ...
      let point_list = point_lists[0];
      let get = 100000;
      if (check >= get && point_list && point_list.total6 != 0) {
        await connection.query(
          "UPDATE users SET money = money + ? WHERE phone = ? ",
      ...
      } else if (check < get && point_list && point_list.total6 != 0) {
        return res.status(200).json({
          message: "Please Recharge ₹ 100000 to claim gift.",
          status: false,
          timeStamp: timeNow,
        });
      } else if (point_list && point_list.total6 == 0) {
        return res.status(200).json({
          message: "You have already received this gift",
          status: false,
          timeStamp: timeNow,
    ...
    if (data == 7) {
      ...
      let point_list = point_lists[0];
      let get = 200000;
      if (check >= get && point_list && point_list.total7 != 0) {
        await connection.query(
          "UPDATE users SET money = money + ? WHERE phone = ? ",
      ...
      } else if (check < get && point_list && point_list.total7 != 0) {
        return res.status(200).json({
          message: "Please Recharge ₹200000 to claim gift.",
          status: false,
          timeStamp: timeNow,
        });
      } else if (point_list && point_list.total7 == 0) {
        return res.status(200).json({
          message: "You have already received this gift",
          status: false,
          timeStamp: timeNow,
...
const withdrawal3 = async (req, res) => {
  ...
  const [user] = await connection.query(
    "SELECT `id_user`, `name_user`, `phone`, `code`, `invite`, `money` FROM users WHERE `token` = ? AND password = ?",
    [auth, md5(password)]
  );
  ...
  if (user_bank.length != 0) {
      ...
      if (userInfo.money - money >= 0) {
        if (result == 0) {
          if (total - total2 >= 0) {
            ...
            // 1. Insert into withdraw table with processed half
            ...
            // 2. Deduct full requested money from user's balance
            await connection.query(
              "UPDATE users SET money = money - ? WHERE phone = ?",
              [money, userInfo.phone] // Note: full money, not just half
            );

            // 3. Add held half to referral_withdraw table
            ...
            if (existingReferral.length > 0) {
              await connection.query(
                "UPDATE referral_withdraw SET referral_amount = referral_amount + ? WHERE phone = ?",
                [halfMoney, userInfo.phone]
              );
            } else {
              await connection.query(
                "INSERT INTO referral_withdraw (phone, referral_amount) VALUES (?, ?)",
                [userInfo.phone, halfMoney]
              );
            }

            // ✅ New insert for tracking full info (does NOT affect existing logic)
            await connection.query(
              `INSERT INTO referral_withdraw (
    user_id,
    name,
    phone,
    referral_amount,
    withdrow_money
              ...
              [
                userInfo.id_user, // use 0 if id not available
                userInfo.name_user, // name
                userInfo.phone,
                halfMoney, // referral_amount (held)
                halfMoney, // withdrow_money (sent to bank)
              ]
            );

            // 4. Send response
            return res.status(200).json({
              message: `Withdrawal successful. ₹${halfMoney} processed to bank, ₹${halfMoney} held in referral wallet.`,
              status: true,
              money: userInfo.money - money, // Deduct full amount from shown balance
              timeStamp: timeNow,
            });
          }
        } else {
...
//                   today = ?,
//                   time = ?`;
//                       await connection.execute(sql, [id_time + '' + id_order, userInfo.phone, halfMoney, infoBank.stk, infoBank.name_bank, infoBank.email, infoBank.name_user, 0, checkTime, dates]);
...
const listReferralWithdraw = async (req, res) => {
  let auth = req.cookies.auth;
  if (!auth) {
    return res.status(200).json({
      message: "Failed",
...
// console.log("userPhone", userPhone);
  ...
  const [history] = await connection.query(
    `SELECT id, receiver_number, sender_number, amount, date
     FROM referral_amount_history
     WHERE receiver_number = ? OR sender_number = ?
     ORDER BY id DESC`,
    [userPhone, userPhone]
  );

  return res.status(200).json({
    message: "Referral Transfer History",
    datas: history,
    phone: userPhone, // 👈 Send this to frontend
    status: true,
    timeStamp: timeNow,
  });
};
