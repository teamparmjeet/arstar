...
async function funHanding(game) {
  ...
  // xử lý game Tổng số
  const [totalNumber] = await connection.execute(
  ...
  const [totaltwoSame] = await connection.execute(
  ...
  const [ThreeSame] = await connection.execute(
  ...
  for (let i = 0; i < ThreeSameL; i++) {
    ...
    if (sult[0].length > 0 && sult[1].length > 0) {
      ...
      let result = ["111", "222", "333", "444", "555", "666"].includes(
        String(kq)
      );
      if (!result) {
        check2 = true;
  ...
  const [Unlike] = await connection.execute(
  ...
  for (let i = 0; i < Unlikes; i++) {
    ...
    if (sult[0].length > 1 && sult[1] == "u" && sult[2].length > 1) {
      ...
      let kq3 = kq[0] + kq[2];
      for (let i = 0; i < kq.length; i++) {
        let resultA1 = ["11", "22", "33", "44", "55", "66"].includes(
          String(kq1)
        );
        let resultA2 = ["11", "22", "33", "44", "55", "66"].includes(
          String(kq2)
        );
        let resultA3 = ["11", "22", "33", "44", "55", "66"].includes(
          String(kq3)
        );
...
const GetMyEmerdList = async (req, res) => {
  ...
  const [result_5d] = await connection.query(
    `SELECT * FROM result_k3 WHERE phone = ? AND game = '${game}' ORDER BY id DESC LIMIT ${
