...
const manageBanners = async (req, res) => {
    return res.render("manage/manageBanners.ejs");
}
...
const incomePage = async (req, res) => {
    return res.render("manage/incomePage.ejs")
}
...
const getIncomePage = async (req, res) => {
    ...
    const [data] = await connection.query('SELECT * FROM income')
    return res.status(200).json({
        message: 'Success',
        status: true,
        datas: data,
...
const rechargeDuyetOld24Jan2024 = async (req, res) => {
    let auth = req.cookies.auth;
    let id = req.body.id;
    let type = req.body.type;
    if (!auth || !id || !type) {
...
const rechargeDuyet = async (req, res) => {
    ...
    if (type == 'confirm') {
        ...
        const [rowsData] = await connection.query(`SELECT COUNT(*) AS count FROM recharge WHERE phone = ? AND status = 1`, [info[0].phone]);
        const countRowsData = rowsData[0].count;

        let moneyData = info[0].money;

        // ----------------------------------------------------------------------------------
        
        // if (countRowsData == "1" && info[0].money > 999) {
        //     moneyData = (info[0].money * 5) / 100;
        //     moneyData = info[0].money + moneyData;
        // }
        let percentage = moneyData * 0.05
        moneyData = moneyData + percentage // adding 5 % of the recharge amount to the actual amount;

        // refferal income 5% increament

        let phone_no = info[0].phone
        let response1 = await connection.query(`SELECT invite, id_user FROM users WHERE phone = ?`, [phone_no]);
        if (countRowsData == '1') {
            
            console.log("Response : ", response1[0][0])
            await connection.query(`UPDATE users SET money = money + ?, total_money = total_money + ? WHERE code = ?`, [percentage, percentage, response1[0][0].invite])
    
            // income table insertion
            let response2 = await connection.query('SELECT id_user FROM users WHERE code = ?', [response1[0][0].invite])
            let type = 'Refferal', level_no =1, remark = `recharge id : ${id}`
            await connection.query('INSERT INTO income (user_id, type, amount, level_no, ref_id, remark) VALUES (?, ?, ?, ?,? ,?);', [response2[0][0].id_user, type, percentage, level_no, response1[0][0].id_user, remark])
        ...
        // income table insertion
        let response2 = await connection.query('SELECT id_user FROM users WHERE code = ?', [response1[0][0].invite])
        let type = 'Cashback', level_no = 0, remark = `recharge id : ${id}`
        await connection.query('INSERT INTO income (user_id, type, amount, level_no, ref_id, remark) VALUES (?, ?, ?, ?,? ,?);', [response1[0][0].id_user, type, percentage, level_no, "NA", remark])



        // END ---------------------------------------------------------------------------------- 

        await connection.query('UPDATE users SET money = money + ?, total_money = total_money + ? WHERE phone = ? ', [moneyData, moneyData, info[0].phone]);
        return res.status(200).json({
            message: 'Successful application confirmation',
            status: true,
            datas: recharge,
...
const settingBank = async (req, res) => {
    ...
    let qr = req.body.qr;
    console.log(name_bank, name, info);
    ...
    if (typer == 'bank') {
        ...
        const data = await connection.query(`UPDATE bank_recharge SET name_bank = ?, name_user = ?, stk = ? WHERE type = 'bank'`, [name_bank, name, info]);
        return res.status(200).json({
    ...
    if (typer == 'momo') {
        const data = await connection.query(`UPDATE bank_recharge SET name_bank = ?, name_user = ?, stk = ?, qr_code_image = ? WHERE type = 'upi'`, [name_bank, name, info, qr]);
        return res.status(200).json({
            message: 'Successful change',
            status: true,
            datas: recharge,
...
const settingUploadQr = async (req, res) => {
    try {
        let auth = req.cookies.auth;
        let qr_file = req.file;
        ...
        if (!qr_file) {
            return res.status(200).json({
                message: 'Failed to upload QR file',
                status: false,
                timeStamp: new Date(),
            });
        }
        ...
        const [result] = await connection.query(
            'UPDATE bank_recharge SET qr_file = ? WHERE type = "upi"',
            [qr_file.filename]
        );
        ...
        console.error('Error updating the QR file:', error);
