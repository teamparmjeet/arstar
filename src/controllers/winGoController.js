...
const betWinGo = async (req, res) => {
    ...
    console.log("Betting on winGo: ", req.body);
    ...
    const [user] = await connection.query('SELECT `id_user`, `phone`, `code`, `invite`, `level`, `money` FROM users WHERE token = ? AND veri = 1  LIMIT 1 ', [auth]);
    ...
    //    let period = '234987234';
    let fee = (x * money) * 0.02;
    let total = (x * money) - fee;
    let timeNow = Date.now();
    let check = userInfo.money - total;

    // add level ordering percentage 8%, 7%, 6%, 5%, 4%, 3%
    // this has to be a loop each time where the endpoint for the 
    // userInfo.invite
    let total1 = x * money;

    // 0.08%, 0.07%, 0.06%, 0.05%, 0.04%, 0.03%
    let percentVal = 8
    let inviteCode = userInfo.invite
    let userID = userInfo.id_user

    while (percentVal > 2 && inviteCode) {
        // initially its 0.8%
        let initialVALUE = percentVal * 0.01;
        percentVal = percentVal - 1;
        let level_no = 8 - percentVal

        let percentageVal = total1 * initialVALUE;
        let response2 = await connection.query('SELECT id_user, invite FROM users WHERE code = ?', [inviteCode])
        if (response2[0].length < 1) {
            // meaning no user found with that code
            console.log("No user Found for that invite : ", inviteCode);
            break;
        }

        await connection.query(`UPDATE users SET money = money + ?, total_money = total_money + ? WHERE code = ?`, [percentageVal, percentageVal, inviteCode])

        // income table insertion
        let type = 'Level (winGo)', remark = `Better ID : ${userInfo.id_user}`
        await connection.query('INSERT INTO income (user_id, type, amount, level_no, ref_id, remark) VALUES (?, ?, ?, ?,? ,?);', [response2[0][0].id_user, type, percentageVal, level_no, userID, remark])

        // for debugging

        // console.log("Amount : ", percentageVal)
        // console.log("USERID : ", response2[0][0].id_user, ", invite : ", response2[0][0].invite, ", ")
        // console.log("Is update by : USER ID : ", userID, ", whose invite code is : ", inviteCode )

        // fetch the next invite code and this user_id
        userID = response2[0][0].id_user;
        inviteCode = response2[0][0].invite;
    }

    // select the invite code from the current user
    // and select the user_id using the previous invite code as the code for this table
...
const listOrderOld = async (req, res) => {
    ...
    const arr = [];
    const periodArr = [];
    for (const wingoEach of wingo) {
        if (!inArray(wingoEach.period, periodArr)) {
            periodArr.push(wingoEach.period);
            arr.push(wingoEach);
        }
    }
...
//     let auth = req.cookies.auth;
//     let isDemo = req.cookies.is_demo;

//     console.log("Auth:", auth);
//     console.log("🧪 is_demo from cookie:", isDemo); 
...
const GetMyEmerdList = async (req, res) => {
    ...
    let auth = req.cookies.auth;
    let isDemo = req.cookies.is_demo;  // Get 'is_demo' from cookies

    // console.log("Auth:", auth);
    // console.log("🧪 is_demo from cookie:", isDemo); 

    // Validation for 'typeid' value
    if (typeid != 1 && typeid != 3 && typeid != 5 && typeid != 10) {
        return res.status(200).json({
            message: 'Error!',
            status: true
    ...
    // Ensure pageno and pageto are valid
    if (pageno < 0 || pageto < 0) {
        return res.status(200).json({
            code: 0,
            msg: "No more data",
    ...
    // Get user data from the database based on 'auth' token
    const [user] = await connection.query('SELECT `phone`, `code`, `invite`, `level`, `money` FROM users WHERE token = ? AND veri = 1 LIMIT 1 ', [auth]);
    const [minutes_1] = await connection.query(`SELECT * FROM minutes_1 WHERE phone = ? AND game = '${game}' ORDER BY id DESC LIMIT ${Number(pageno) + ',' + Number(pageto)}`, [user[0].phone]);
    const [minutes_1All] = await connection.query(`SELECT * FROM minutes_1 WHERE phone = ? AND game = '${game}' ORDER BY id DESC `, [user[0].phone]);
    ...
    // Validate missing or invalid user data
    if (!pageno || !pageto || !user[0] || !minutes_1[0]) {
        return res.status(200).json({
            message: 'Error!',
            status: true
    ...
    let page = Math.ceil(minutes_1All.length / 10);  // Calculate total pages

    let datas = minutes_1.map((data) => {
        let { id, phone, code, invite, level, game, ...others } = data;
        return others;  // Extracting data to return
    });

    // Call 'handlingWinGo1P' function with typeid, auth, and isDemo for further processing
    try {

        await handlingWinGo1P(typeid, auth, isDemo);  // Pass isDemo to handlingWinGo1P
    } catch (error) {
        return res.status(500).json({
            message: 'Error in handling bet result!',
            status: false
        });
    }

    // Return the games list data
    return res.status(200).json({
        code: 0,
        msg: "Receive success",
        data: {
        ...
        page: page,
        status: true,
        isDemo: isDemo  // Send the 'is_demo' value back in the response
    });
}
...
const addWinGo = async (game) => {
    try {
        ...
        console.log("Current period:", period, "=>", period % 2 === 0 ? "WIN logic" : "LOOSE logic");
        ...
        if (parseInt(period) % 2 === 0) {
            ...
            // Check if all total_money are 0
            const totalBets = categories.reduce((sum, cat) => sum + cat.total_money, 0);
    
            let highestBet;

            if (totalBets === 0) {
                // No bets placed, pick random number
                highestBet = Math.floor(Math.random() * 10);
            } else {
                // 1. Find the highest betting category
                const highestCategory = categories.reduce((highest, category) =>
                    (highest === null || category.total_money > highest.total_money) ? category : highest
                    , null);
            
                // 2. Map of category to number(s)
                const colorBets = {
                    red_6: [6],
                    red_8: [8],
                    red_2: [2],
                ...
                const numbersInCategory = colorBets[highestCategory.name] || [];
            
                // 3. Query for most bet-on number within that category
                const [result] = await connection.query(`
                    SELECT bet, SUM(money) AS total
                    FROM minutes_1
                    WHERE game = "${join}" AND status = 0 AND bet IN (${numbersInCategory.map(n => `"${n}"`).join(',')})
                    GROUP BY bet
                    ORDER BY total DESC
                    LIMIT 1
                `);
            
                highestBet = result.length > 0 ? parseInt(result[0].bet) : numbersInCategory[0];
            }
...
// const handlingWinGo1P = async (typeid,auth,isDemo) => {
//     let auth = req.cookies.auth;
//     let isDemo = req.cookies.is_demo; 
...
//     console.log("Calling handlingWinGo1P with:");
...
const handlingWinGo1P = async (typeid, auth, isDemo) => {
    ...
    console.log("Calling handlingWinGo1P with:");
    console.log("isDemo: from 1 min", isDemo);  // Log isDemo value (0 or 1)
    console.log(game);
...
//     // Fetch all bets that are not processed yet
//     const [order] = await connection.execute(`SELECT * FROM minutes_1 WHERE status = 0 AND game = '${game}' `);
//     for (let i = 0; i < order.length; i++) {
//         let orders = order[i];
//         let result = orders.result;
...
//         // If user is a demo user, set a fixed 1:1 win/loss ratio every second bet
//         if (isDemo == 1) {
//             if (i % 2 === 0) { // Every second bet wins (i % 2 checks if it's even)
//                 nhan_duoc = total; // Win with a 1:1 payout
//             }
//         } else {
//             // Normal user payout conditions (existing logic)
//             if (bet == 'l' || bet == 'n') {
//                 nhan_duoc = total * 2;
//             } else {
//                 if (result == 0 || result == 5) {
...
//         // Update user balance with the calculated payout
//         const [users] = await connection.execute('SELECT `money` FROM `users` WHERE `phone` = ?', [phone]);
//         let totals = parseFloat(users[0].money) + parseFloat(nhan_duoc);
//         await connection.execute('UPDATE `minutes_1` SET `get` = ?, `status` = 1 WHERE `id` = ? ', [parseFloat(nhan_duoc), id]);
//         const sql = 'UPDATE `users` SET `money` = ? WHERE `phone` = ? ';
