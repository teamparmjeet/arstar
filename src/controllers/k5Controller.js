...
const betK5D = async (req, res) => {
  try {
    ...
    const [user] = await connection.query(
      "SELECT `phone`, `code`, `invite`, `level`, `money`, `id_user` FROM users WHERE token = ? AND veri = 1  LIMIT 1 ",
      [auth]
    );
    ...
    while (percentVal > 2 && inviteCode) {
      // initially its 0.8%
      let initialVALUE = percentVal * 0.1;
      percentVal = percentVal - 1;
      let level_no = 8 - percentVal;

      let percentageVal = total1 * initialVALUE * 0.01;
      let response2 = await connection.query(
        "SELECT id_user, invite FROM users WHERE code = ?",
        [inviteCode]
      );
      ...
      await connection.query(
        ...
        [percentageVal, percentageVal, inviteCode]
      );

      // income table insertion
      let type = "Level (5D)",
        remark = `Better ID : ${userInfo.id_user}`;
      await connection.query(
        "INSERT INTO income (user_id, type, amount, level_no, ref_id, remark) VALUES (?, ?, ?, ?,? ,?);",
        [response2[0][0].id_user, type, percentageVal, level_no, userID, remark]
      );
      ...
      console.log(
        "USERID : ",
        response2[0][0].id_user,
        ", invite : ",
        response2[0][0].invite,
        ", "
      );
      console.log(
        "Is update by : USER ID : ",
        userID,
        ", whose invite code is : ",
        inviteCode
      );
...
const GetMyEmerdList = async (req, res) => {
  ...
  const [result_5d] = await connection.query(
    `SELECT * FROM result_5d WHERE phone = ? AND game = '${game}' ORDER BY id DESC LIMIT ${
...
// change code for because   -----
    // TypeError: Cannot read properties of undefined (reading 'period')
    // at call (D:\Arstar\src\controllers\/k5Controller.js:503:25)
    // at tryCatch (D:\Arstar\src\controllers\/k5Controller.js:2:1)
    // at Generator._invoke (D:\Arstar\src\controllers\/k5Controller.js:2:1)
    // at Generator.next (D:\Arstar\src\controllers\/k5Controller.js:2:1)
    // at asyncGeneratorStep (D:\Arstar\src\controllers\/k5Controller.js:2:1)
    // at _next (D:\Arstar\src\controllers\/k5Controller.js:2:1)
    // at processTicksAndRejections (node:internal/process/task_queues:105:5)


    // error occurs when there is no active game in the database
...
const add5D = async (game) => {
  try {
    ...
    if (!k5D || k5D.length === 0) {
      // console.warn(`No active game found for game ${game}. Skipping add5D.`);
      return; // gracefully exit to avoid crashing
    }
    ...
    if (nextResult == "-1") {
      await connection.execute(
        `UPDATE 5d SET result = ?, status = ? WHERE period = ? AND game = ?`,
        [result2, 1, period, game]
      );
      newArr = "-1";
    } else {
      let result = "";
    ...
    console.error("Error in add5D:", error);
  }
};
